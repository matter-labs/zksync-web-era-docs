...
Verifying a contract entails determining whether the on-chain bytecode equals to the smart contract source code given to the verifier.

## How are smart contracts verified in zkSync?

The deployed bytecode and the Solidity source code of a smart contract are compared during the verification process using zkSync.
The algorithm compiles the given source code to compare the generated bytecode to the deployed one.
<br>
...
You can verify your contract so that the [zkSync block explorer](https://explorer.zksync.io/) can convert it all into a format that is readable by everyone. It is beneficial if you want to make the most of 
our zkEVM Debugger capabilities.
