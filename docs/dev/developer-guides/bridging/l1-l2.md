...
To prevent users from requesting too many heavy operations from the operator, users must pay a fee (in ETH) to the zkSync operator. Thus, each 
transaction must pay an L1 fee and an L2 fee. 
...
- `_ergsLimit` is a parameter that contains the ergs limit of the transaction call. You can learn more about ergs and the zkSync fee system [here](../fee-model.md).
