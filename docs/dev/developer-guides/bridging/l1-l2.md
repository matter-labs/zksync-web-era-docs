...
::: warning
- After the system update released in Feb 2023, the `ergs` concept is only used by the VM while the API layer operates with `gas`. 
- For more information, check the [changelog](../../troubleshooting/changelog.md).
:::
...
If the `wait()` function takes much longer than expected, most likely the transaction has failed.