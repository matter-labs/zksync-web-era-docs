...
A "baseFee" for a transaction, means the minimum amount the user has to pay the operator to include the transaction. It is derived based on the `l2gasLimit` for a transaction and the gas price on L1.

:::tip
`l2gasLimit` is the maximum amount of L2 gas that a transaction can consume during execution on L2.
:::

At the moment, all the L1-> L2 transactions are served at the first-in-first-out basis, but in the future we will introduce "priority heap", which will allow for sorting the transactions.
...
3. Send the transaction including the required `value`.
...
`eth_estimateGas` is applicable for both zkSync Era L2 and Ethereum while `zks_estimateGasL1ToL2` is only available on zkSync Era.

::: warning
 If the sender is a smart contract, then during the L1->L2 the `msg.sender` address is aliased. Note that this is not done automatically during fee estimation, so it is your responsibility to use the zkSync Era [`aliasing utility`](#aliasing) to get the correct `from` address for the fee estimation.
:::
...
You can use the `estimateGasL1` to estimate the L2 gas needed for the L1->L2 transaction.
...
To interact with the zkSync Era [mailbox contract](https://github.com/matter-labs/v2-testnet-contracts/blob/main/l1/contracts/zksync/facets/Mailbox.sol) using Solidity, you need to use the zkSync Era contract interface. There are two main ways to get it:
