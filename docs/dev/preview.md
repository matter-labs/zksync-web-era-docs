...
- **Native support of ECDSA signatures.** Unlike the first version of zkSync and most of the zkRollups no special operation is required to register the user's private key. Any account can be managed in L2 with the same private key that is used for L1.
