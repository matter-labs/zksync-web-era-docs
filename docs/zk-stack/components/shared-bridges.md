...
infrastructure that is focused on an ecosystem of interconnected zkEVMs, which we call ZK Chains. This ecosystem
...
[ZK Chains](../concepts/zk-chains.md) in our docs.
...
- Some assets have to be natively supported (ETH, WETH) and it also makes sense to support some generally accepted token
  ...
  the zkSync ecosystem). These canonical asset contracts are deployed from L1 by a bridge shared by all ZK Chains. This is
