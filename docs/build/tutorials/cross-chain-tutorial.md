...
- Find [more info about the zkSync CLI here]().
:::
...
async function main() {
  ...
  // For more information on `REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_LIMIT` see the [fee model documentation]().
...
:::tip
- Executing transactions from L1 requires the caller to pay a fee to the L2 operator. The fee depends on the length of the calldata and the `gasLimit`. This is similar to the `l2gasLimit` on Ethereum. You can read more about the [zkSync fee model here]().
