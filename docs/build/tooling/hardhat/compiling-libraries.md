...
### 1 Using hardhat-zksync-solc version >= 0.4.2
...
To address the error, you can follow the instructions provided in the output, which is the recommended approach. For more details, please refer to [this section](#_1-automatic-deployment).
Alternatively, if you prefer a manual resolution for the libraries, you can find detailed instructions in [this section](#_2-manual-deployment).

Choose the method that best suits your preferences or requirements.

### 2 Using hardhat-zksync-solc version < 0.4.2
...
That error tells us that the address of the `MiniMath` library should be provided in the compilation process.

To resolve the error, it's necessary to manually resolve the libraries. For detailed instructions, please refer to [this section](#_1-automatic-deployment).

## Non-inline librareis deployment

### 1. Automatic deployment

::: note
This approach is effective only with `hardhat-zksync-solc` version equal to or greater than 0.4.2, also, `hardhat-zksync-deploy` version must be equal to or greater than 0.6.5.
