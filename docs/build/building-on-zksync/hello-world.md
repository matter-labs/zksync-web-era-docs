...
Now visit the [zkSync block explorer](https://explorer.zksync.io/) and search with the contract address to confirm the deployment. Follow the [contract verification guide]() for instructions on how to verify your smart contract using the zkSync block explorer.
...
4. Now, what is left is to encode the paymasterInput following the [protocol requirements]() and return the needed overrides. 
