...
Although this plugin works out of the box, zkSync Era has a [different fee model](../concepts/fee-model.md) than Ethereum. Users should consider this when analysing the report generated by this plugin.

In addition, make sure to read about [local testing](../../development/test-and-debug/README.md).
...
However, we do not recommend using the Hardhat network for testing contracts that will be deployed on zkSync Era. We recommend instead using the [in-memory node](../test-and-debug/era-test-node/getting-started.md) or the [docker setup to test your contracts](../test-and-debug/era-test-node/dockerized-testing.md) as they will give you the same results as our testnet/mainnet.
