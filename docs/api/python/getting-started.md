...
Full code of all examples are available [here](https://github.com/zksync-sdk/zksync2-python/tree/master/examples). Examples are configured to interact with `zkSync` and `Goerli` test networks.  

### Deposit funds

This is an example of how to deposit assets from Ethereum network (L1) to zkSync network (L2):
...
Here is an example how to check balance on zkSync network
```python
from eth_account import Account
from eth_account.signers.local import LocalAccount
...
if __name__ == "__main__":
    ...
    if not len(WITHDRAW_TX_HASH):
        ...
        print("When withdraw transaction is finalized, execute this script again "
...
There are 3 solidity smart contracts that will be deployed:
...
- [Demo](https://github.com/zksync-sdk/zksync2-python/blob/master/examples/solidity/demo/Demo.sol): contract that has dependency to 
[Foo](https://github.com/zksync-sdk/zksync2-python/blob/master/examples/solidity/demo/Foo.sol) contract.

There is a [user guide](https://github.com/zksync-sdk/zksync2-python/blob/master/examples/README.md) on how to compile Solidity smart contract using `zksolc` 
compiler. `zksolc` compiler generates `combined.json` file that contains bytecode and ABI of smart contract. Those files are used in following examples. 

#### Deploy a contract

To deploy the contract, contract ABI is needed for calling its methods in the standard web3 way.
`ConctractEncoder` is used to read ABI and bytecode from the `combined.json` file. 
...
### Deploy a contract with constructor and make interaction with it

In some cases, you would need to get the contract address before deploying it. Use `PrecomputeContractDeployer` 
to get precomputed contract address. Incrementer contract does not need precomputed address, but it is used in
this example to check does precomputed address match with deployed address. This example also shows how to
...
### Deploy a smart contract with dependency using create2
