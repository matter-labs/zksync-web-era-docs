...
export const checkFrontmatter = (page, isDebug) => {
    ...
    if (isDebug) {
        ...
        // check sidebar
        if ("sidebar" in frontmatter &&
            frontmatter.sidebar !== "heading" &&
            typeof frontmatter.sidebar !== "boolean") {
            logger.warn(`"sidebar" property in Page FrontMatter should be "heading" or boolean.${filePathRelative ? `\nFound in ${filePathRelative}` : ""}`);
            delete frontmatter.sidebar;
        }
        // check string values
        ["title", "shortTitle", "containerClass"].forEach((key) => {
            if (key in frontmatter && typeof frontmatter[key] !== "string") {
                logger.warn(`"${key}" property in Page FrontMatter should be string.${filePathRelative ? `\nFound in ${filePathRelative}` : ""}`);
        ...
        [
            ...
            if (key in frontmatter && typeof frontmatter[key] !== "boolean") {
                logger.warn(`"${key}" property in Page FrontMatter should be boolean.${filePathRelative ? `\nFound in ${filePathRelative}` : ""}`);
                delete frontmatter[key];
            }
        });
    }
