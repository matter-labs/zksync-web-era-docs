...
export const themeConfig = (themeConfig) => {
    ...
    return convertThemeOptions(themeConfig);
};
const checkMarkdownOptions = (options = {}) => {
    // lineNumbers
    if ("lineNumbers" in options) {
        logger.warn(`${colors.magenta("markdown.lineNumbers")} is ${colors.yellow("deprecated")}  in VuePress2, please use ${colors.magenta("markdown.code.lineNumbers")} instead.`);
    ...
    if ("slugify" in options) {
        logger.error(`\
${colors.magenta("markdown.slugify")} is ${colors.red("no longer supported")} in VuePress2.
...
`);
    ...
    if ("pageSuffix" in options) {
        logger.error(`${colors.magenta("markdown.pageSuffix")} is ${colors.red("no longer supported")} in VuePress2.`);
        delete options["pageSuffix"];
    }
    // externalLinks
    if ("externalLinks" in options) {
        logger.error(`${colors.magenta("markdown.externalLinks")} is ${colors.red("no longer supported")} in VuePress2, please use ${colors.magenta("markdown.links.externalAttrs")} instead.`);
        delete options["externalLinks"];
    }
    // plugins
    if ("plugins" in options) {
        logger.error(`${colors.magenta("markdown.plugins")} is ${colors.red("no longer supported")} in VuePress2, please use ${colors.magenta("extendsMarkdown")} hook instead.`);
...
const checkPluginOptions = (plugins) => {
            ...
            if (typeof item === "object") {
                ...
                [
                    ...
                    if (deprecatedOption in item)
                        logger.warn(`${colors.magenta(deprecatedOption)} options in plugin options is ${colors.yellow("deprecated")} in VuePress2, please use ${colors.magenta(newOption)} instead.`);
                ...
                [
                    ...
                    if (removedOption in item)
                        logger.error(`${colors.magenta(removedOption)} option in plugin options is ${colors.red("no longer supported")} in VuePress2, make sure you are using a VuePress2 plugin.`);
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    ...
    if (typeof plugins === "object") {
        logger.error(`${colors.magenta('object format "plugins"')} is ${colors.red("no longer supported")} in VuePress2, you should import plugins and call them in an array.`);
        return [];
    }
    return [];
};
export const checkBundlerOptions = (config) => {
    [
        ...
        if (removedOption in config)
            logger.error(`"${colors.magenta(removedOption)}" option in config file is ${colors.red("no longer supported")} in VuePress2, you should set it in bundler options.`);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
...
export const config = (userConfig) => {
    ...
    [
        ...
        if (deprecatedOption in userConfig)
            logger.warn(`"${deprecatedOption}" option in config file is ${colors.yellow("deprecated")} in VuePress2, please use "${newOption}" instead.`);
        // eslint-disable-next-line
    ...
    [
        ...
        if (removedOption in userConfig)
            logger.error(`"${removedOption}" option in config is ${colors.red("no longer supported")} in VuePress2${hint ? `, ${hint}.` : "."}`);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        delete userConfig[removedOption];
    });
    // other options
    if ("extraWatchFiles" in userConfig) {
        logger.error(`${colors.magenta("extraWatchFiles")} options is ${colors.red("removed")} in VuePress2, you should use "onWatched" hook.`);
