...
Текущая модель имеет несколько важных минусов: она не позволяет кастомным кошелькам отправлять множество транзакций одновременно, сохраняя при этом детерминистический порядок. Для ЕОА-аккаунтов nonce'ы должны увеличиваться последовательно, тогда как для кастомных аккаунтов порядок транзакций не может быть определен на 100%.
...
* Логика аккаунта может взаимодействовать только со слотами хранилища, принадлежащими данному аккаунту. Учтите, что [определение принадлежности](podderzhka-abstrakcii-akkaunta.md#extending-the-set-of-slots-that-belong-to-a-user) выходит далеко за рамки слотов на адресе пользователя.
