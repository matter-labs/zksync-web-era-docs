...
В Эфириуме существует два типа аккаунтов: [аккаунты, находящиеся во внешнем владении (ЕОА)](https://ethereum.org/en/developers/docs/accounts/#externally-owned-accounts-and-key-pairs), и [аккаунты смарт-контрактов](https://ethereum.org/en/developers/docs/accounts/#contract-accounts). Только первый тип может инициировать транзакции, в то время как только второй может реализовывать произвольную логику. Для некоторых случаев использования, таких как кошельки в виде смарт-контрактов или протоколы конфиденциальности, эта разница может создавать много трений. В результате они требуют ретрансляторов L1, то есть EOA, чтобы облегчить транзакции из смарт-контракт-кошелька.
...
* Перед началом транзакции система делает запрос к [NonceHolder](../sistemnye-kontrakty/#inonceholder) для проверки был ли предоставленный nonce уже использован.
...
Для каждого аккаунта рекомендуется применять интерфейс [IAccount](https://github.com/matter-labs/v2-testnet-contracts/blob/main/l2/system-contracts/interfaces/IAccount.sol).
...
Как и в оригинальном EIP4337, наш протокол абстракции аккаунта поддерживает paymasters: аккаунты, которые могут возмещать стоимость транзакций других аккаунтов. Вы можете больше узнать о них [здесь](./#paymasters).&#x20;
...
**Шаг 1.** Система проверяет, не был ли nonce транзакции использован ранее. Вы можете прочитать больше о сохранении nonce уникальным [здесь](./#keeping-nonces-unique).
...
В целях оптимизации, и[ nonce транзакции и nonce развертывания](../../razrabotka-na-zksync/razvertyvanie-kontrakta/#differences-in-create-behaviour) сохраняются в один слот хранилища внтури системного контракта [NonceHolder](../sistemnye-kontrakty/#inonceholder). Для увеличения nonce вашего аккаунта крайне рекомендуется вызывать функцию [incrementNonceIfEquals](https://github.com/matter-labs/v2-testnet-contracts/blob/0e1c95969a2f92974370326e4430f03e417b25e7/l2/system-contracts/interfaces/INonceHolder.sol#L10) и передать значение nonce, предоставленное в транзакции.
...
Paymaster поддерживает только [approval-based](./#poryadok-ispolneniya-paymaster-osnovannyi-na-razreshenii-tratit-tokeny-approval-allowance) порядок исполнения paymaster'ов и требует, чтобы параметр `token` был равен обмениваемому токену, а `minAllowance` был равен как минимум `tx.maxFeePerErg * tx.ergsLimit`.
